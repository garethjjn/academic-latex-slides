...
1. **Run the style linter:** `python ../../scripts/lint_style.py <draft-file>` — fix EVERY ERROR and EVERY WARN (blacklist verbs; contribution-literature; triadic). CRITICAL: never write "results show that" / "show that" / "demonstrate" in claim sentences; use "we find" / "Table N reports" / "the results indicate". Re-run until **0 ERRORs and 0 WARNs** (em-dash density excepted if genuinely parenthetical).
2. **Run the structure checker:** `python ../../scripts/check_structure.py <draft-file> --section results` — it mechanically verifies sub-section coverage, that an **identification sub-section is present**, and that **identification precedes cross-sectional** (item 3 below). Resolve every `N`.
