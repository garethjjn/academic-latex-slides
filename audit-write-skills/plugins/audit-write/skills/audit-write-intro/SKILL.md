...
2. **Run the structure checker:** `python ../../scripts/check_structure.py <draft-file> --section intro` — resolve any `N`: 5-block coverage (motivation/gap · theory/tension · setting/data · findings · robustness · contributions); Block-4 magnitude (an honest `[PLACEHOLDER: ...]` is fine pre-results); and **signed-prediction-as-formal-H** — if you make a directional prediction ("we posit X increases Y"), it must be crystallized as a displayed `**H1.** … ceteris paribus` statement at the end of Block 2 (a pure RQ-first / exploratory intro with no signed prediction is exempt and reads `NA`).
